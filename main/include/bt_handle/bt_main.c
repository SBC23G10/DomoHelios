...
static uint8_t *cpy_addr;
static uint8_t *fi_addr;
static const char *tag = "SBC23G10H3"; // Device name(as seen by connected devices)
...
void init_bt()
{
    ...
    xTaskCreate(&send, "send", 2048, NULL, 5, NULL);
    // Create and start advert task
    //xTaskCreatePinnedToCore(&btAdvtTask, "btAdvtTask", 2048, NULL, 5, NULL, 1);
    xTaskCreate(&btAdvtTask, "btAdvtTask", 2048, NULL, 5, NULL);
}

void set_cpy_addr(unsigned char *_fi_addr, unsigned char *_cpy_addr)
{
    fi_addr = _fi_addr;
    cpy_addr = _cpy_addr;
}